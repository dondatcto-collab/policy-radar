...
v1.1 — nếu market data cũ hơn phiên giao dịch gần nhất (cron tối chưa chạy lại được):
  KHÔNG hiện mục Dòng tiền, KHÔNG hiện đề xuất trạng thái (kể cả trong bảng radar
  gửi cho dashboard) — lớp dòng tiền lỗi thời thì tín hiệu kép (CLAUDE.md #2) không
  còn giá trị. Tin Telegram chỉ còn cờ ⚠️ + dòng khóa, không hiện cả tin chính sách."""
from utils import CONFIG, load_json, save_json, data_path, today_str, now_vn
from health_check import check, du_lieu_cu
...
def build():
    ...
    noi_bat, tin, de_xuat = [], [], []
    radar_nhom = radar.get("nhom", {})
    if cu:
        # Data cũ: bảng radar gửi dashboard cũng không được lộ đề xuất/mã chú ý cũ
        radar_nhom = {nid: {**n, "de_xuat_may": n.get("trang_thai_nguoi_duyet"), "ma_dang_chu_y": []}
                      for nid, n in radar_nhom.items()}
    else:
    ...
    lines += flags[:2]
    if cu:
        lines.append("🔒 Radar tạm khóa do dữ liệu cũ — chờ cron tối chạy lại")
    else:
        if noi_bat:
